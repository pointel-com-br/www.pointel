...
2) Identifique a pessoa (1a, eu, nós; 2a, tu, vós; 3a, eie(a), eles(a))
e o número do verbo
(singular/plural).
...
:(A) "Ou você, Joãoz deseja alguma coisa?" (14° parágrafo)

I

:(B) "Por obséquio, me acompanhe até a sala VIP." (6o parágrafo)

:(C) "Posso esperar perfeitamente aqui mesmo." (7o parágrafo)

I

:(D) "Vivemos numa república, João." (23° parágrafo)
...
:Quem surgiu? Os bancos "surgiram", então "os bancos" é sujeito de
"surgirem". Gabarito letra:
...
:Na oração "mas podem serlimitados", o sujeito não apareceu expressamente porque já foi:

imencionado antes e está claro no contexto:
:

'.Os direitos humanos são fundados no respeito peia dignidade e no valor de cada pessoa. (Os',
...
É necessário o uso debarcos > O uso de barcos é necessário.
:
...
I: (B) seu Adelino veio sentar-se ao lado da antiga freguesa
...
: Temos "virDE+AÍ" (vir dai} e "vir DE+A ignorância" (vir da ignorância).
...
: Ademais, em segundo plano, tal atribuição físcalizatória advém dos preceitos morais que impõem
...
iDentre as expressões destacadas, a que exerce a mesma função sintática do segmento:

isublinhado em "Stephanie Preston, professora de psicologia da Universidade de Michigan, nos:
...
iclínica, prefere falar em 'emoções desreguladas' do que 'negativas'.".
:
...
:(D) "A paleta de cores emocionais engloba emoções desreguladas. como tristeza,
frustração,:
iraiva, ansiedade ou inveja.".
:
...
ilocução adjetiva, sintaticamenteum adjunto adnominal. O termo "professor" não pede:
icomplemento.
:
...
:é objeto indireto!). Em D, temos objeto direto. Em Ez temos adjunto adverbial de tempo.
...
:Os termos "a crimes contra o público" e "dequeixas" complementam, respectivamente, os:
itermos "relativos" e "investigações".
:
...
:"lnvestigações", por sua vez, é um substantivo abstrato derivado de ação e "de queixas" possui:

ivalor passivo: "queixas são investigadas". Então, temos clássico caso de complemento nominal.:
iQuestão correta.
:
...
:Os termos "de gênero", "da igualdade racial" e "dosdireitos humanos"complementam a:
palavra "justiça".
:
...
:Os termos "da igualdade racial" e "dosdireitos humanos"complementam a palavra"garantia".:

:São termos preposicionados passivos ligados a substantivo abstrato derivado de ação:

iGarantia "da igualdade racial" (a igualdade racial é garantida) e

I
I

iGarantia "dos direitos humanos" (os direitos humanos são garantidos)
:

:O termo preposicionado "de gênero" não possui sentido passivo, é uma especificação, apenas:
lum adjunto adnominal de "justiça". Questão incorreta.
:
...
icomplementos diretos da forma verbal "temos".
:
...
:Os termos "um amigo" e"preparados" exercem amesmafunção sintáticanos períodos em que:
...
'.condutores, técnicos deenfermagem,enfermeiros emédicos estãopreparados. Questão:
lincorreta.
:
...
:De que adiantaria tornaraleimais rigorosa...
:
...
:Com relação aos aspectoslinguísticosdo texto, julgue o seguinte item.
:
...
:Aqui, o verbo "tornar-se" está sendoutilizado como verbo transitivo direto. A estrutura é;
Tomar1.
'.Xalguma coisa; ou seja, tem um objeto direto e esse objeto vai receber um predicativo:
...
:"Estável" e "movediça" atribuem qualidade ao sujeito, por via de um verbo ligação,
"é", o que:
inão ocorre com "cultural". Temos, então, dois predicativos do sujeito.
:
...
ela é:
lestável emovediça. Como vimos, isso confirma a função de adjunto adnominal.
:
...
---- Ah, professor! Porque não posso dizer que "da Pipa" é um adjunto adnominal?
...
lesclarecimento. O termo "o preço eficiente dos bens e serviços" é justamente o esclarecimento:
...
lum nome comum. Então, notamos que "Oscar Wilde" é um aposto especificativo do substantivo:
icomum "iconoclasta".
:
...
imodificado peloadjetivo"iconoclasta",com função de adjunto adnominal.
:
...
: "A par disso, quando se pensa no processo eleitoral — embora logo venha à cabeça a figura doã.
...
iverbal nos períodos em que ocorrem.
:
...
iradar" (L.1-2), fosse empregada a preposição por, em vez da preposição "de".
:
...
» pois para calar esse feijão, soprar nele.
...
t« obstrui a leitura íluviante. flutuai.

açu la a atenção, isca-a como o risco.
...
is Também nâo tem amigos em lugares próximos.
...
r pois para calar esse feijão, soprar nele.
e jogar fora o leve e oco. palha e eco.
