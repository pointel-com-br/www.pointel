...
Tópico. Parâmetros que o servidor nginx aceita com -s. 
...
1. `stop`: Encerra o processo principal do Nginx de forma limpa, permitindo que as conexões ativas sejam concluídas antes de encerrar o servidor.

2. `quit`: Encerra o processo principal do Nginx imediatamente, sem esperar pelas conexões ativas serem concluídas. Este comando é mais abrupto do que o comando `stop`.
